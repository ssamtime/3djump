...
public class OffMeshLinkClimb : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        while(true)
        {
            // IsOnClimb() 함수의 반환값이 true일 때 까지 반복 호출
    ...
    public bool IsOnClimb()
    {
        ...
        if(navMeshAgent.isOnOffMeshLink)
        {
            ...
            // 현재 위치에 있는 OffMeshLink가 수동으로 생성한 offMeshLink이고
            // 장소 정보가 "Climb"이면
    ...
    private IEnumerator ClimbOrDesend()
    {
        ...
        // 현재 위치에 있는 OffMeshLink의 시작/종료 위치를 가져온다.
        OffMeshLinkData linkData = navMeshAgent.currentOffMeshLinkData;
        ...
        // OffMeshLink 이동이 완료 되었으니 네비게이션을 이요한 이동을 다시 시작한다.
